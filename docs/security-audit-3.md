...
> **关于轮次记录**：本仓库只保留第二、三两次审计的完整记录。**第一轮与第四轮没有独立文档**，
> 它们的处置写在引用它们的地方（附提交号），例如下文的「处置（第四轮，commit `5376ca3`）」；
> 需要核对时以那些提交与守卫为准，而不是假定存在一份可以打开的第四轮报告。

**方法**：按不变量分组的对抗性探针清单，三个**并行、严格只读**的对抗子代理各领一处。
一条发现的成立标准不变：
