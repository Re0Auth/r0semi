...
这些只在 CI 或发布时运行，**不链接进任何二进制**，所以不改变上面的依赖图。列在这里是因为“为什么是它”
同样需要一个答案，而且**版本是固定的**——发布与门禁不该漂在别人的 tag 上。
...
组件数设了下限（反空转），因为一个**没有任何组件**的 SBOM 是一份合法文档，能通过其它所有检查。

## 7. 供应链的边界（明确不做）

发布链路钉住的是**我们能控制的那一半**。把剩下的一半写清楚，是为了让"有 SBOM、有签名"不被读成
"整条链都可验证"：
...
- **这条边界怎么读**：SBOM 与签名回答"这份产物出自这条流水线、由这些模块构成"，
  它们**不**回答"这些模块本身可信吗"。后者依赖 Go 模块校验和与人工审查，不在本仓库自动化的范围内；
  要升级，需要先开一次决策（例如引入 `slsa-github-generator` 或对输入做 provenance 校验），
  而不是在某次发版时顺手加上一个声明。
