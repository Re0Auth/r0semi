...
- **能挡什么，不能挡什么**（写在迁移注释里，因为**夸大的完整性控制比没有更糟**）：
  ...
  - **从尾部删行 → 就地验证挡不住**。截断在链头没有被记到库外时不可见——尾部只是变短，剩余每一环都仍成立。**服务自己每小时（以及每次启动）把链头写进日志**（组合根里的 `anchorLoop`，日志行是 `audit chain head anchored head=<hex>`）：日志管线是另一个系统、有自己的保留期，而且不需要凭据——`GET /v1/admin/audit/head` 按设计要运维会话，定时任务拿不到它。核对步骤（拿日志里最新的锚点去表里找那一行）见 [operations.md](./operations.md#审计链锚点核对)；**日志不落到库外的部署，维持原有的较弱保证**。导出侧由 `GET /v1/admin/audit` 读 API 承担（供 SIEM 拉取）。
